| Name                         | Version     | License                                           | URL
                                       |
|------------------------------|-------------|---------------------------------------------------|---------------------------------------------------------------------------------------------------|
| GitPython                    | 3.1.45      | BSD-3-Clause                                      | https://github.com/gitpython-developers/GitPython
                                       |
| Jinja2                       | 3.1.6       | BSD License                                       | https://github.com/pallets/jinja/
                                       |
| MarkupSafe                   | 3.0.3       | BSD-3-Clause                                      | https://github.com/pallets/markupsafe/
                                       |
| altair                       | 5.5.0       | BSD License                                       | https://github.com/vega/altair
                                       |
| annotated-types              | 0.7.0       | MIT License                                       | https://github.com/annotated-types/annotated-types
                                       |
| attrs                        | 25.4.0      | MIT                                               | https://www.attrs.org/en/stable/changelog.html
                                       |
| beautifulsoup4               | 4.14.3      | MIT License                                       | https://www.crummy.com/software/BeautifulSoup/bs4/
                                       |
| blinker                      | 1.9.0       | MIT License                                       | https://github.com/pallets-eco/blinker/
                                       |
| branca                       | 0.8.2       | MIT License                                       | https://github.com/python-visualization/branca
                                       |
| cachetools                   | 6.2.2       | MIT License                                       | https://github.com/tkem/cachetools/
                                       |
| certifi                      | 2025.11.12  | Mozilla Public License 2.0 (MPL 2.0)              | https://github.com/certifi/python-certifi
                                       |
| cffi                         | 2.0.0       | MIT                                               | https://cffi.readthedocs.io/en/latest/whatsnew.html        
                                       |
| charset-normalizer           | 3.4.4       | MIT                                               | https://github.com/jawah/charset_normalizer/blob/master/CHANGELOG.md                              |
| click                        | 8.3.1       | BSD-3-Clause                                      | https://github.com/pallets/click/
                                       |
| colorama                     | 0.4.6       | BSD License                                       | https://github.com/tartley/colorama
                                       |
| curl_cffi                    | 0.13.0      | MIT License                                       | https://github.com/lexiforest/curl_cffi
                                       |
| defusedxml                   | 0.7.1       | Python Software Foundation License                | https://github.com/tiran/defusedxml
                                       |
| folium                       | 0.20.0      | MIT License                                       | https://github.com/python-visualization/folium
                                       |
| fonttools                    | 4.61.0      | MIT                                               | http://github.com/fonttools/fonttools
                                       |
| fpdf2                        | 2.8.5       | LGPL-3.0-only                                     | https://py-pdf.github.io/fpdf2/
                                       |
| frozendict                   | 2.4.7       | GNU Lesser General Public License v3 (LGPLv3)     | https://github.com/Marco-Sulla/python-frozendict
                                       |
| geographiclib                | 2.1         | MIT                                               | https://geographiclib.sourceforge.io/Python/
                                       |
| geopy                        | 2.4.1       | MIT License                                       | https://github.com/geopy/geopy
                                       |
| gitdb                        | 4.0.12      | BSD License                                       | https://github.com/gitpython-developers/gitdb
                                       |
| google-ai-generativelanguage | 0.6.15      | Apache Software License                           | https://github.com/googleapis/google-cloud-python/tree/main/packages/google-ai-generativelanguage |
| google-api-core              | 2.28.1      | Apache Software License                           | https://github.com/googleapis/python-api-core
                                       |
| google-api-python-client     | 2.187.0     | Apache Software License                           | https://github.com/googleapis/google-api-python-client/    
                                       |
| google-auth                  | 2.43.0      | Apache Software License                           | https://github.com/googleapis/google-auth-library-python   
                                       |
| google-auth-httplib2         | 0.2.1       | Apache Software License                           | https://github.com/GoogleCloudPlatform/google-auth-library-python-httplib2                        |
| google-generativeai          | 0.8.5       | Apache Software License                           | https://github.com/google/generative-ai-python
                                       |
| googleapis-common-protos     | 1.72.0      | Apache Software License                           | https://github.com/googleapis/google-cloud-python/tree/main/packages/googleapis-common-protos     |
| grpcio                       | 1.76.0      | Apache Software License                           | https://grpc.io
                                       |
| grpcio-status                | 1.71.2      | Apache Software License                           | https://grpc.io
                                       |
| httplib2                     | 0.31.0      | MIT License                                       | https://github.com/httplib2/httplib2
                                       |
| idna                         | 3.11        | BSD-3-Clause                                      | https://github.com/kjd/idna
                                       |
| jsonschema                   | 4.25.1      | MIT                                               | https://github.com/python-jsonschema/jsonschema
                                       |
| jsonschema-specifications    | 2025.9.1    | MIT                                               | https://github.com/python-jsonschema/jsonschema-specifications                                    |
| multitasking                 | 0.0.12      | Apache Software License                           | https://github.com/ranaroussi/multitasking
                                       |
| narwhals                     | 2.13.0      | MIT License                                       | https://github.com/narwhals-dev/narwhals
                                       |
| numpy                        | 2.2.6       | BSD License                                       | https://numpy.org
                                       |
| packaging                    | 25.0        | Apache Software License; BSD License              | https://github.com/pypa/packaging
                                       |
| pandas                       | 2.3.3       | BSD License                                       | https://pandas.pydata.org
                                       |
| peewee                       | 3.18.3      | MIT License                                       | https://github.com/coleifer/peewee/
                                       |
| pillow                       | 12.0.0      | MIT-CMU                                           | https://python-pillow.github.io
                                       |
| platformdirs                 | 4.5.0       | MIT                                               | https://github.com/tox-dev/platformdirs
                                       |
| plotly                       | 6.5.0       | MIT License                                       | https://plotly.com/python/
                                       |
| proto-plus                   | 1.26.1      | Apache Software License                           | https://github.com/googleapis/proto-plus-python
                                       |
| protobuf                     | 5.29.5      | 3-Clause BSD License                              | https://developers.google.com/protocol-buffers/
                                       |
| pyarrow                      | 21.0.0      | Apache Software License                           | https://arrow.apache.org/
                                       |
| pyasn1                       | 0.6.1       | BSD License                                       | https://github.com/pyasn1/pyasn1
                                       |
| pyasn1_modules               | 0.4.2       | BSD License                                       | https://github.com/pyasn1/pyasn1-modules
                                       |
| pycparser                    | 2.23        | BSD License                                       | https://github.com/eliben/pycparser
                                       |
| pydantic                     | 2.12.5      | MIT                                               | https://github.com/pydantic/pydantic
                                       |
| pydantic_core                | 2.41.5      | MIT                                               | https://github.com/pydantic/pydantic-core
                                       |
| pydeck                       | 0.9.1       | Apache License 2.0                                | https://github.com/visgl/deck.gl/tree/master/bindings/pydeck                                      |
| pyparsing                    | 3.2.5       | MIT                                               | https://github.com/pyparsing/pyparsing/
                                       |
| python-dateutil              | 2.9.0.post0 | Apache Software License; BSD License              | https://github.com/dateutil/dateutil
                                       |
| pytz                         | 2025.2      | MIT License                                       | http://pythonhosted.org/pytz
                                       |
| referencing                  | 0.37.0      | MIT                                               | https://github.com/python-jsonschema/referencing
                                       |
| requests                     | 2.32.5      | Apache Software License                           | https://requests.readthedocs.io
                                       |
| rpds-py                      | 0.30.0      | MIT                                               | https://github.com/crate-py/rpds
                                       |
| rsa                          | 4.9.1       | Apache Software License                           | https://stuvel.eu/rsa
                                       |
| six                          | 1.17.0      | MIT License                                       | https://github.com/benjaminp/six
                                       |
| smmap                        | 5.0.2       | BSD License                                       | https://github.com/gitpython-developers/smmap
                                       |
| soupsieve                    | 2.8         | MIT                                               | https://github.com/facelessuser/soupsieve
                                       |
| streamlit                    | 1.51.0      | Apache Software License                           | https://streamlit.io
                                       |
| streamlit-folium             | 0.25.3      | UNKNOWN                                           | UNKNOWN
                                       |
| tenacity                     | 9.1.2       | Apache Software License                           | https://github.com/jd/tenacity
                                       |
| toml                         | 0.10.2      | MIT License                                       | https://github.com/uiri/toml
                                       |
| tornado                      | 6.5.2       | Apache Software License                           | http://www.tornadoweb.org/
                                       |
| tqdm                         | 4.67.1      | MIT License; Mozilla Public License 2.0 (MPL 2.0) | https://tqdm.github.io
                                       |
| typing-inspection            | 0.4.2       | MIT                                               | https://github.com/pydantic/typing-inspection
                                       |
| typing_extensions            | 4.15.0      | PSF-2.0                                           | https://github.com/python/typing_extensions
                                       |
| tzdata                       | 2025.2      | Apache Software License                           | https://github.com/python/tzdata
                                       |
| uritemplate                  | 4.2.0       | BSD 3-Clause OR Apache-2.0                        | https://uritemplate.readthedocs.org
                                       |
| urllib3                      | 2.5.0       | MIT                                               | https://github.com/urllib3/urllib3/blob/main/CHANGES.rst   
                                       |
| watchdog                     | 6.0.0       | Apache Software License                           | https://github.com/gorakhargosh/watchdog
                                       |
| websockets                   | 15.0.1      | BSD License                                       | https://github.com/python-websockets/websockets
                                       |
| xlsxwriter                   | 3.2.9       | BSD License                                       | https://github.com/jmcnamara/XlsxWriter
                                       |
| xyzservices                  | 2025.11.0   | BSD License                                       | https://github.com/geopandas/xyzservices
                                       |
| yfinance                     | 0.2.66      | Apache Software License                           | https://github.com/ranaroussi/yfinance

<br>

## Fonts & Assets
This project includes the following font software:

| Name | Copyright Holder | License | URL |
|---|---|---|---|
| NanumGothic | Copyright (c) 2010, NHN Corporation | SIL Open Font License 1.1 | https://hangeul.naver.com/font |

<br>

---

### NanumGothic License (SIL OFL 1.1)

Copyright (c) 2010, NHN Corporation (http://www.nhncorp.com),
with Reserved Font Name Nanum, Naver Nanum, NanumGothic, ...

This Font Software is licensed under the SIL Open Font License, Version 1.1.
This license is copied below, and is also available with a FAQ at:
https://openfontlicense.org

[OFL.txt](./test/app/fonts/NanumGothic/OFL.txt)
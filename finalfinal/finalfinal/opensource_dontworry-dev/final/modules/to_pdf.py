...
def create_pdf(df):
    ...
    font_path = os.path.join(app_dir, "fonts", "NotoSans", "NotoSansKR-VF.ttf")
    ...
    if os.path.exists(font_path):
        pdf.add_font(family="NotoSans", fname=font_path)
        pdf.set_font("NotoSans", size=10)
    else:
        # 폰트가 없으면 기본 폰트 사용 (한글 깨짐 주의)
        pdf.set_font("Arial", size=10)
        st.toast("⚠️ NotoSans 폰트 파일이 없어 한글이 깨질 수 있습니다.", icon="⚠️")

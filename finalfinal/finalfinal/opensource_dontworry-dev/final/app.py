...
# 1. 페이지 설정
st.set_page_config(page_title="AI 여행 가계부", page_icon="✈️", layout="wide")
st.title("Don't ₩orry 😽")
st.subheader("AI 스마트 여행 가계부")
...
with st.sidebar:
    ...
    selected_date = st.date_input("환전 기준 날짜", datetime.today())
...
with tab1:
    ...
    with col1: # --- 영수증 업로드 ---
        ...
        if uploaded_file:
            ...
            st.image(image, caption='영수증 이미지', use_container_width=True)
    ...
    with col2: # --- 입력/수정 폼 ---
        if st.session_state.expense_edit_id:
            ...
            if uploaded_file and api_key and st.button("영수증 분석하기"):
        ...
        with st.form("expense_form"):
            ...
            input_price = st.number_input(f"금액 ({currency})", min_value=0.0, format="%.2f", step=0.01,value=st.session_state.form_price)
    ...
    if not expenses_df.empty:
        ...
        d_col1, d_col2, d_col3, empty_space = st.columns([2.2, 2.4, 2.2, 6.925])
...
with tab2:
    ...
    with m_col2:
        ...
        if st.session_state.memory_edit_id is not None:
            ...
            # 2. [추가된 로직] 만약 위에서 '지출'을 선택했다면? -> 그 지출의 날짜를 가져옴!
            if sel_exp_id is not None:
                # expenses_df에서 선택된 ID에 해당하는 줄 찾기
                target_row = expenses_df[expenses_df['id'] == sel_exp_id]
                
                if not target_row.empty:
                    # DB에 저장된 날짜 문자열("2024-05-20")을 가져옴
                    date_str = target_row.iloc[0]['date']
                    try:
                        # 문자열을 날짜 객체로 변환하여 기본값 덮어쓰기
                        def_date = datetime.strptime(date_str, "%Y-%m-%d")
                    except:
                        pass # 날짜 변환 실패 시 그냥 오늘 날짜 사용
